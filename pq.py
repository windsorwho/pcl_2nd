...
class PQ(object):
    ...
    def __init__(self, M, Ks=256, verbose=True):
        ...
        if verbose:
            print("M: {}, Ks: {}, code_dtype: {}".format(
                M, Ks, self.code_dtype))

    def __eq__(self, other):
        if isinstance(other, PQ):
            return (self.M, self.Ks, self.verbose, self.code_dtype,
                    self.Ds) == (
                        other.M,
                        other.Ks,
                        other.verbose,
    ...
    def fit(self, vecs, iter=20, seed=123):
        ...
        for m in range(self.M):
            ...
            self.codewords[m], _ = kmeans2(vecs_sub,
                                           self.Ks,
                                           iter=iter,
                                           minit="points")
    ...
    def decode(self, codes):
        ...
        for m in range(self.M):
            vecs[:, m * self.Ds:(m + 1) *
                 self.Ds] = self.codewords[m][codes[:, m], :]
    ...
    def dtable(self, query):
        ...
        for m in range(self.M):
            ...
            dtable[m, :] = np.linalg.norm(self.codewords[m] - query_sub,
                                          axis=1)**2
