...
def test_encoding_error(out_length=32,min_val=0.8,max_val=5.0):
    ...
    coder = sparse_coder.SparseVector(out_dim=out_length,min_val=min_val, max_val=max_val)
    error = []
    for x in features:
        x_code = coder.sparsify(x)
        x_hat = coder.densify(x_code)
        error.append(np.linalg.norm(x-x_hat))
    print('MSE for length %d (%f,%f): %f' %(out_length,min_val,max_val,np.mean(np.array(error))))     
